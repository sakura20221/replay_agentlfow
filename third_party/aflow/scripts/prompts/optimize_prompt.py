...
```
**You MUST define every `prompt_custom.XXXX` variable you reference in the graph, inside the `<prompt>` section. For example, if the graph uses `prompt_custom.SOLVE_PROMPT`, then `<prompt>` must contain `SOLVE_PROMPT = "..."`. `XXX_PROMPT` above is a placeholder for a name you choose, not a variable that exists.**
