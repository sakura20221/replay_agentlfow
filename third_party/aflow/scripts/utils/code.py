...
def extract_test_cases_from_jsonl(entry_point: str, dataset: Union[CodeDataset, str] = CodeDataset.HUMAN_EVAL, problem: str = None):
    # 统一获取 dataset 的字符串值
    dataset_value = dataset.value if isinstance(dataset, CodeDataset) else dataset

    file_map = {
        "SHARED_MBPP": "data/datasets/shared_mbpp_public_test.jsonl",  # shared-layer shim
