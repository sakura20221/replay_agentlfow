...
import CARD.prompt.shared_prompt_sets  # noqa: F401  # registers the shared domains
...
def parse_args():
    parser = argparse.ArgumentParser(description="CARD Experiments on gsm8k")
    parser.add_argument(
        "--dataset_json", type=str, default="datasets/gsm8k/gsm8k.jsonl"
    )
...
_DOMAIN_DEFAULTS = {'math': (['MathSolver'], [5], 'FinalRefer'), 'code': (['CodeWriting'], [5], 'FinalWriteCode'), 'qa': (['AnalyzeAgent'], [5], 'FinalRefer')}
...
async def main():
    ...
    result_dir = Path(f"{CARD_ROOT}/result/{args.domain}")
    ...
    # --- shared-layer shim: supply the combination dict CARD requires ---
    ...
    for i_batch in range(num_batches):
        ...
        for i_record, record in enumerate(current_batch):
            ...
            answer_log_probs.append(
                asyncio.create_task(realized_graph.arun(
