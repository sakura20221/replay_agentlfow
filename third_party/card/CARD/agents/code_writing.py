    ...
    def _process_inputs(
        ...
        **kwargs,
        ...
        for id, info in spatial_info.items():
            if (
                info["output"].startswith("```python")
                and info["output"].endswith("```")
                and self.role != "Normal Programmer"
        ...
        user_prompt += (
            f"In the last round of dialogue, the outputs and feedbacks of some agents were: \n\n{temporal_str}"
            if len(temporal_str)
            else ""
        )
