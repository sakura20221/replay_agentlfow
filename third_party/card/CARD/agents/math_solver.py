...
from CARD.tools.coding.python_executor import execute_code_get_return
    ...
    def _process_inputs(
        ...
        **kwargs,
        ...
        user_prompt = self.prompt_set.get_answer_prompt(
            question=raw_inputs["task"], role=self.role
        )
        if self.role == "Math Solver":
            ...
            user_prompt += (
                f"In the last round of dialogue, there were the following responses to the same question for your reference: \n\n{temporal_str}"
                if len(temporal_str)
                else ""
            )
