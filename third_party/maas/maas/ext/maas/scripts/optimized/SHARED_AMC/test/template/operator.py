...
from maas.ext.maas.scripts.optimized.MATH.train.template.operator_an import *
from maas.ext.maas.scripts.optimized.MATH.train.template.op_prompt import *
...
class GenerateCoT(Operator):
    ...
    async def __call__(self, input, instruction):
        prompt = _shim_safe_format(GENERATE_COT_PROMPT, instruction=instruction, input=input)
        response = await self._fill_node(GenerateOp, prompt, mode="single_fill")
        return response
...
class MultiGenerateCoT(Operator):
    ...
    async def __call__(self, input, instruction):
        prompt = _shim_safe_format(GENERATE_COT_PROMPT, input=input,instruction=instruction)
...
class Programmer(Operator):
    ...
    async def code_generate(self, problem, analysis, feedback, mode):
        prompt = _shim_safe_format(PYTHON_CODE_VERIFIER_PROMPT, 
            problem=problem,
            analysis=analysis,
            feedback=feedback
        )
