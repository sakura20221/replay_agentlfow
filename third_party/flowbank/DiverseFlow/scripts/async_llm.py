...
class LLMConfig:
    def __init__(self, config: dict):
        ...
        self.base_url = config.get("base_url", "<your base url>")
...
class LLMsConfig:
    ...
    def default(cls):
        ...
        if cls._default_config is None:
            # Look for the active config (copy config/config.example.yaml ->
            # config/config.yaml and fill in your API keys). Resolved relative to
            # both the current dir and this DiverseFlow package dir so `run.py`
            # works regardless of where it is launched from.
            pkg_dir = Path(__file__).resolve().parent.parent      # .../DiverseFlow
            names = ["config/config.yaml", "config.yaml",
                     "config/config2.yaml", "config2.yaml"]
            config_paths = [Path(n) for n in names] + [pkg_dir / n for n in names]
            ...
            if config_file is None:
                raise FileNotFoundError(
                    "No DiverseFlow LLM config found. Copy "
                    "config/config.example.yaml to config/config.yaml and add your "
                    "API keys (searched: config/config.yaml, config.yaml, "
                    "config/config2.yaml).")
    ...
    def get(self, llm_name: str) -> LLMConfig:
        ...
        llm_config = {
            "model": config.get("model_name", llm_name),  # Use model_name if specified, else key
            "temperature": config.get("temperature", 1),
            "key": config.get("api_key"),  # Map api_key to key
            "base_url": config.get("base_url", "<your base url>"),
...
class AsyncLLM:
    ...
    async def __call__(self, prompt):
        ...
        # Build extra_body for models that need special parameters (e.g., disable Qwen3 thinking mode)
        extra_body = {}
...
def create_llm_instance(llm_config):
    ...
    if _shim_ns_url:
        try:
            llm_config.base_url = _shim_ns_url
