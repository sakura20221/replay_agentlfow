...
class Custom(Operator):
    ...
    async def __call__(self, input=None, instruction="", **kwargs):
        # Accept 'problem' as alias for 'input' (LLM often confuses the two)
        if input is None:
            input = kwargs.get("problem", "")
        # Support placeholder replacement if instruction contains {input} or {problem}
        if "{input}" in instruction or "{problem}" in instruction:
            prompt = instruction.replace("{input}", input).replace("{problem}", input)
        else:
...
class Programmer(Operator):
    def __init__(self, llm: AsyncLLM, name: str = "Programmer"):
        ...
        # Lock to protect process pool rebuild from concurrent access
    ...
    async def _rebuild_pool(self, kill_workers=False):
        """Rebuild the process pool under a lock to avoid concurrent rebuilds.

        When one task times out and kills the worker, other concurrent tasks
        waiting on the same pool hit BrokenProcessPool. The lock ensures only
        one rebuild happens; others wait for the fresh pool.
        """
        async with self._pool_lock:
            try:
                if kill_workers:
                    for proc in self.process_pool._processes.values():
                        proc.kill()
    ...
    async def exec_code(self, code, timeout=20):
        """
        Asynchronously execute code and return an error if timeout occurs.
        """
        loop = asyncio.get_running_loop()

        for attempt in range(2):  # retry once on BrokenProcessPool
            try:
                ...
                future.cancel()
                # Original approach: cancel future + gc (but stuck workers survive and block exit)
                # import gc
                # gc.collect()
                # Fix: kill stuck worker processes and rebuild the pool to avoid zombie workers
                await self._rebuild_pool(kill_workers=True)
                return "Error", "Code execution timed out"
            except asyncio.CancelledError:
                # Another task's timeout killed the shared pool, cancelling our future.
                # Rebuild and retry once so this task gets a fair chance.
                await self._rebuild_pool(kill_workers=False)
                if attempt == 0:
                    continue
                return "Error", "Task was cancelled due to pool rebuild"
            except concurrent.futures.process.BrokenProcessPool:
                # Pool was killed by another task's timeout. Rebuild and retry once
                # so this task gets a fair chance to execute on the fresh pool.
                await self._rebuild_pool(kill_workers=False)
                if attempt == 0:
                    continue  # retry with the fresh pool
                return "Error", "Process pool broken, try again"
            except Exception as e:
                return "Error", f"Unknown error: {str(e)}"
        return "Error", "Max exec_code retries reached"
...
class QANumerical(Programmer):
    """DROP/QA-oriented code-execution operator.

    Reuses Programmer's process pool / retry / sandbox infrastructure but uses
    a prompt tailored for reading comprehension questions with numerical or
    structural reasoning (counting, arithmetic, sorting, set ops, date diffs).

    Unlike Programmer which assumes the input is a math problem, QANumerical
    accepts a (passage + question) string as `problem` and allows the generated
    code to hard-code extracted values from the passage.
    """

    def __init__(self, llm: AsyncLLM, name: str = "QANumerical"):
        super().__init__(llm, name)

    async def code_generate(self, problem, analysis, feedback, mode):
        """Override to use QA_NUMERICAL_PROMPT instead of PYTHON_CODE_VERIFIER_PROMPT."""
        prompt = QA_NUMERICAL_PROMPT.format(
            problem=problem,
            analysis=analysis,
            feedback=feedback,
        )
...
def _run_test_case(test_code):
    """Run a single test case in a separate process."""
    exec(test_code, {})


class Test(Operator):
    def __init__(self, llm: AsyncLLM, name: str = "Test", dataset=None):
        super().__init__(llm, name)
    ...
    def exec_code(self, problem, solution, entry_point, timeout=30):
        test_cases = extract_test_cases_from_jsonl(entry_point, dataset=self.dataset, problem=problem)

        fail_cases = []
        for test_case in test_cases:
            test_code = test_case_2_test_function(solution, test_case, entry_point)
            try:
                future = self._pool.submit(_run_test_case, test_code)
                future.result(timeout=timeout)
            except concurrent.futures.TimeoutError:
                # Worker is stuck, replace the pool
                self._pool.shutdown(wait=False, cancel_futures=True)
                self._pool = concurrent.futures.ProcessPoolExecutor(max_workers=1)
                return {"exec_fail_case": f"Code execution timed out after {timeout}s"}
...
class Format(Operator):
    ...
    async def __call__(self, problem, solution, mode: str = None):
        ...
        return response.get("solution", str(response))
