import asyncio
import os
import threading
import time
from concurrent.futures import ThreadPoolExecutor
from typing import Any, Callable, Dict, List, Optional, Tuple

from tenacity import retry, retry_if_exception_type, stop_after_attempt, wait_fixed

from benchmarks.benchmark import BaseBenchmark
from scripts.logs import logger

# Shared thread pool for code execution across all problems.
# Limits parallelism to avoid excessive CPU/memory usage.
_CODE_EXEC_POOL = ThreadPoolExecutor(
    max_workers=min(16, os.cpu_count() or 4),
    thread_name_prefix="cf_exec",
)


class CodeforcesBenchmark(BaseBenchmark):
    """Codeforces competitive programming benchmark (stdin/stdout style).

    Follows the HumanEval/MBPP evaluation pattern.
    Test logic uses pre-generated check(candidate) functions that mock
    stdin and capture stdout for comparison.
    """

    def __init__(self, name: str, file_path: str, log_path: str, llm_config=None):
        super().__init__(name, file_path, log_path, llm_config)

    class TimeoutError(Exception):
        pass

    def run_with_timeout(self, func, args, timeout):
        result = []
        stop_event = threading.Event()

        def target():
            try:
                result.append(func(*args))
            except Exception as e:
                result.append(e)
            finally:
                stop_event.set()

        thread = threading.Thread(target=target)
        thread.start()
        is_timeout = not stop_event.wait(timeout)

        if is_timeout:
            raise self.TimeoutError("Function execution timed out")

        if not result:
            return None
        if isinstance(result[0], Exception):
            raise result[0]
        return result[0]

    def wrap_solution_code(self, code):
        """Wrap generated code into a def solution(): function.

        Codeforces code is script-style (reads stdin, writes stdout),
        so it must be wrapped into a function for check(candidate) to call.
        If the code already defines a solution function, skip wrapping.
        """
        # Strip markdown code block markers
        if "```python" in code:
            code = code.split("```python")[-1]
            code = code.split("```")[0]
        elif "```" in code:
            code = code.split("```")[1]
            code = code.split("```")[0]

        code = code.strip()

        # Check if solution function is already defined
        if "def solution(" in code or "def solution()" in code:
            return code

        # Separate imports from body code
        lines = code.split("\n")
        import_lines = []
        body_lines = []
        for line in lines:
            stripped = line.strip()
            if stripped.startswith("import ") or stripped.startswith("from "):
                import_lines.append(line)
            else:
                body_lines.append(line)

        # Build wrapped code
        parts = []
        if import_lines:
            parts.append("\n".join(import_lines))
            parts.append("")

        parts.append("def solution():")
        if not body_lines or all(line.strip() == "" for line in body_lines):
            parts.append("    pass")
        else:
            for line in body_lines:
                if line.strip() == "":
                    parts.append("")
                else:
                    parts.append("    " + line)

        return "\n".join(parts)

    def check_solution(self, solution, test, entry_point):
        """Verify that generated code passes all test cases."""
        solution = self.wrap_solution_code(solution)
        try:
            global_dict = {
                "math": __import__("math"),
                "sys": __import__("sys"),
                "re": __import__("re"),
                "collections": __import__("collections"),
                "itertools": __import__("itertools"),
                "functools": __import__("functools"),
                "heapq": __import__("heapq"),
                "bisect": __import__("bisect"),
                "string": __import__("string"),
                "List": List,
                "Dict": Dict,
                "Tuple": Tuple,
                "Optional": Optional,
                "Any": Any,
            }

            exec(solution, global_dict)

            if entry_point not in global_dict:
                raise ValueError(f"Function {entry_point} is not defined in the solution.")

            exec(test, global_dict)

            check = global_dict["check"]

            # 60s timeout (Codeforces problems may have many test cases)
            result = self.run_with_timeout(check, (global_dict[entry_point],), 60)

            if result is None:
                result = (self.PASS, "The solution passed all test cases.")

        except self.TimeoutError:
            result = (
                self.FAIL,
                "Execution timed out. Please check if your solution contains infinite loops or overly time-consuming operations.",
            )
        except Exception as e:
            error_message = f"Error: {str(e)}.\n Solution: {solution[:500]}.\n Test: {test[:200]}"
            result = (self.FAIL, error_message)

            with open("error.log", "a", encoding="utf-8") as log_file:
                log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - {error_message}\n")

        return result

    @retry(stop=stop_after_attempt(5), wait=wait_fixed(1), retry=retry_if_exception_type(Exception), reraise=True)
    async def _generate_output(self, graph, prompt, entry_point):
        return await asyncio.wait_for(graph(prompt, entry_point), timeout=60)

    async def evaluate_problem(self, data: dict, graph: Callable) -> Tuple[str, str, str, float, float]:
        input_text = data["prompt"]
        expected_output = "\nCorrect Solution:\n" + data.get("canonical_solution", "")

        try:
            # Generate code via LLM
            prediction, cost = await self._generate_output(graph, input_text, data["entry_point"])

            # Run check_solution in thread pool to avoid blocking the event loop
            loop = asyncio.get_running_loop()
            ret = await loop.run_in_executor(
                _CODE_EXEC_POOL,
                self.check_solution, prediction, data["test"], data["entry_point"]
            )
            test_case_details = ret[1]
            expected_output = test_case_details + expected_output

            # Score
            score = 1.0 if ret[0] == self.PASS else 0.0

            # Log failures
            if score == 0:
                self.log_mismatch(input_text, expected_output, prediction, score)

            return input_text, prediction, expected_output, score, cost

        except asyncio.TimeoutError:
            logger.info("Timeout error. Skipping this sample.")
            return input_text, "Timeout", expected_output, 0.0, 0.0

        except Exception as e:
            logger.info(f"Maximum retries reached. Skipping this sample. Error: {e}")
            return input_text, str(e), expected_output, 0.0, 0.0

    def calculate_score(self, expected_output: str, prediction: str) -> Tuple[float, str]:
        return 0.0, prediction

    def get_result_columns(self) -> List[str]:
        return ["inputs", "prediction", "expected_output", "score", "cost"]
