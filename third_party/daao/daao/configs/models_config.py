...
from daao.config2 import merge_dict
from daao.configs.llm_config import LLMConfig
from daao.const import CONFIG_ROOT, METAGPT_ROOT
...
class ModelsConfig(YamlModel):
    ...
    Attributes:
        ...
        available_llms (List[str], optional): List of model names available for LLMRouter selection.
                   If not set, defaults to all models in `models` dict.

    Methods:
        update_llm_model(cls, value): Validates and updates LLM model configurations.
        from_home(cls, path): Loads configuration from ~/.maas/config2.yaml.
        ...
        get_available_llms(self) -> List[str]: Returns list of available LLM names for router.
    """

    models: Dict[str, LLMConfig] = Field(default_factory=dict)
    available_llms: Optional[List[str]] = Field(default=None, description="Models available for LLMRouter selection. Defaults to all models if not set.")
    ...
    def from_home(cls, path):
        """
        Loads configuration from ~/.maas/config2.yaml.
    ...
    def get_available_llms(self) -> List[str]:
        """
        Returns list of available LLM names for LLMRouter selection.
        If `available_llms` is set, returns that list.
        Otherwise, returns all model names from `models` dict.

        Returns:
            List[str]: List of available LLM model names.
        """
